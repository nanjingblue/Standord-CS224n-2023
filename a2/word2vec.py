...
def naiveSoftmaxLossAndGradient(
    ...
    '''
    在 word2vec 模型中,我们假设中心词向量和外部词向量之间存在线性相关性。也就是说,中心词向量和外部词向量的点积越大,表示它们越相关
    '''
    y_hat = softmax(outsideVectors @ centerWordVec)
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1   # one-hot

    '''
    Softmax 函数的输出 y_hat 是一个概率分布,表示模型预测每个外部词被选中的概率。
    在 Naive Softmax 损失函数中,我们只关心正确的外部词被选中的概率,也就是 y_hat[outsideWordIdx]。
    由于我们希望最大化正确外部词的概率,因此损失函数采用负对数似然的形式,即 -log(y_hat[outsideWordIdx])。这样做可以将最大化概率转化为最小化损失。
    不需要使用真实标签 y,是因为 Softmax 函数本身就能够很好地表示概率分布。y_hat 中的每个元素已经代表了每个外部词被选中的概率。
    也就是说,通过直接使用 Softmax 输出概率 y_hat[outsideWordIdx],就可以计算出 Naive Softmax 损失,不需要额外的真实标签 y。这种方式计算损失函数更加简洁和高效。
    '''
    loss = -np.log(y_hat[outsideWordIdx])

    # the partial derivative of any function f (x) with respect to x should have the same shape as x
    gradCenterVec = (y_hat - y) @ outsideVectors    # (N, ) @ (N, W) -> (N, )
    gradOutsideVecs = np.outer((y_hat - y), centerWordVec)  # (N, ) · (W, ) -> (H, W)
...
def negSamplingLossAndGradient(
    ...
    negative = outsideVectors[indices, :]  # (outsideWordIdx + K, D)
    negative[1:] *= -1  # 对应公式的 sigmoid(-u_k * v_c) 在计算损失函数时,正样本的向量会增加损失,而负样本的向量会减少损失

    # convince to sum
    score = sigmoid(negative @ centerWordVec)  # (outsideWordIdx + K, D) @ (D, ) -> (outsideWordIdx + K, )
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for idx in outsideWordIdx:
        loss_, gradCenterVec, gradOutsideVec = word2vecLossAndGradient(centerWordVec, idx, outsideVectors, dataset)
        loss += loss_
